...
const els = {
  ...
  surfaceTypePanel: document.getElementById("surfaceTypePanel"),
...
function dieShapeOptionsFromConfig() {
  const general = state.config?.general || {};
  return [
...
function dieShapeOptionValue(value = "") {
  const token = dieShapeToken(value);
  const match = dieShapeOptionsFromConfig().find((item) => dieShapeToken(item.value) === token || dieShapeToken(item.label) === token);
...
function dieShapeThumbForValue(value = "", image = "") {
  const img = image || dieShapeImageForValue(value);
  return img ? `<img src="${esc(img)}" alt="${esc(value || "")}">` : `<span class="calc-shape-thumb-fallback" data-shape="${esc(value || "Rectangular")}"></span>`;
}
...
function shapeSelectOptions(selectedValue = "") {
  const current = dieShapeOptionValue(selectedValue);
  return dieShapeOptionsFromConfig().map((item) => `<option value="${esc(item.value)}"${item.value === current ? " selected" : ""}>${esc(item.label)}</option>`).join("");
}
...
function ensureCalcShapePanel() {
  ...
  const panel = document.createElement("div");
  panel.className = "calc-shape-panel";
  panel.setAttribute("data-calc-shape-panel", "");
  panel.hidden = true;
  shapes.forEach((shape) => {
    const label = document.createElement("label");
    label.className = "calc-shape-option";
    label.setAttribute("data-calc-shape-value", shape.value);
    const radio = document.createElement("input");
    radio.type = "radio";
    radio.name = "calc_die_shape";
    radio.value = shape.value;
    radio.dataset.label = shape.label;
    radio.dataset.image = shape.image;
    label.appendChild(radio);
    const thumb = document.createElement("span");
    thumb.className = "calc-shape-thumb";
    thumb.innerHTML = shape.image
      ? `<img src="${esc(shape.image)}" alt="${esc(shape.label)}">`
      : `<span class="calc-shape-thumb-fallback" data-shape="${esc(shape.value)}"></span>`;
    label.appendChild(thumb);
    const txt = document.createElement("span");
    txt.className = "calc-shape-trigger-label";
    txt.textContent = shape.label;
    label.appendChild(txt);
    panel.appendChild(label);
  });
  panel.addEventListener("click", (event) => {
    const option = event.target.closest?.(".calc-shape-option");
    if (option) {
      const radio = option.querySelector('input[name="calc_die_shape"]');
      if (radio) {
        radio.checked = true;
        syncCalcShapePickerState(radio);
      }
    }
  });
  document.body.appendChild(panel);
  calcShapePanelCache = panel;
  return panel;
}

function positionCalcShapePanel(triggerEl) {
  const panel = ensureCalcShapePanel();
  if (!triggerEl || panel.hidden) return;
  ...
  const maxHeight = Math.max(150, Math.min(400, window.innerHeight - top - gap));
  panel.style.setProperty("--calc-shape-panel-left", `${left}px`);
  panel.style.setProperty("--calc-shape-panel-top", `${top}px`);
  panel.style.setProperty("--calc-shape-panel-width", `${width}px`);
  panel.style.setProperty("--calc-shape-panel-max-height", `${maxHeight}px`);
}
...
function toggleCalcShapePanel(forceOpen) {
  ...
  if (nextState) {
    const selected = state.form?.troquel?.dieShape || "";
    const labels = panel.querySelectorAll(".calc-shape-option");
    labels.forEach((l) => {
      const r = l.querySelector('input[name="calc_die_shape"]');
      if (r) {
        const isMatch = dieShapeToken(r.value) === dieShapeToken(selected);
        r.checked = isMatch;
        l.classList.toggle("is-selected", isMatch);
      }
    });
    positionCalcShapePanel(trigger);
  }
}

function syncCalcShapePickerState(radio) {
  const panel = ensureCalcShapePanel();
  if (radio) {
    state.form.troquel.dieShape = dieShapeOptionValue(radio.value);
    if (state.form.troquel.associateDieShape !== false && state.form.troquel.dieCode) {
      const selectedDie = findDie(state.form.troquel.dieCode);
      if (selectedDie && !dieMatchesShape(selectedDie, state.form.troquel.dieShape)) state.form.troquel.dieCode = "";
    }
    const allLabels = panel.querySelectorAll(".calc-shape-option");
    allLabels.forEach((l) => l.classList.toggle("is-selected", l.querySelector('input[name="calc_die_shape"]')?.checked));
  }
...
function buildSavePayload() {
  ...
  return {
    ...
    surfaceType: state.form.header.surfaceType,
...
function resolveApplicationOptions() {
  return normalizeConfigTextList(state.config?.general?.quoteApplicationOptionsJson, DEFAULT_APPLICATION_OPTIONS);
}

function resolveSurfaceOptions() {
  return normalizeConfigTextList(state.config?.general?.quoteSurfaceOptionsJson, DEFAULT_SURFACE_OPTIONS);
}

function positionCalcInlinePanel(panel, input) {
  ...
  const maxHeight = Math.max(140, Math.min(420, window.innerHeight - top - gap));
  panel.style.setProperty("--calc-inline-left", `${left}px`);
  panel.style.setProperty("--calc-inline-top", `${top}px`);
  panel.style.setProperty("--calc-inline-width", `${width}px`);
  panel.style.setProperty("--calc-inline-max-height", `${maxHeight}px`);
}

function renderCalcTextSuggestions(input, panel, items, emptyMessage) {
  if (!input || !panel) return;
  const term = String(input.value || "").trim().toLowerCase();
  const source = (items || []).map((item) => String(item || "").trim()).filter(Boolean);
  const filtered = source.filter((item) => !term || item.toLowerCase().includes(term)).slice(0, 12);
  if (term && !filtered.length) {
    panel.hidden = true;
    return;
  }
  panel.innerHTML = filtered.length
    ? filtered.map((item) => `<button type="button" class="calc-inline-option" data-calc-inline-value="${esc(item)}">${esc(item)}</button>`).join("")
    : `<div class="calc-inline-empty">${esc(emptyMessage)}</div>`;
  panel.hidden = false;
  positionCalcInlinePanel(panel, input);
}

function showApplicationEnvironmentPanel() {
  renderCalcTextSuggestions(els.applicationEnvironment, els.applicationEnvironmentPanel, resolveApplicationOptions(), "No hay aplicaciones configuradas.");
}

function showSurfaceTypePanel() {
  renderCalcTextSuggestions(els.surfaceType, els.surfaceTypePanel, resolveSurfaceOptions(), "No hay tipos de superficie configurados.");
}

function hideCalcInlinePanels() {
  if (els.applicationEnvironmentPanel) els.applicationEnvironmentPanel.hidden = true;
...
function dieDimensionMismatch() {
  ...
  const rotated = Math.abs(productWidth - dieLength) <= tolerance && Math.abs(productLength - dieWidth) <= tolerance;
  if (direct || rotated) return null;
  return {
    product: `${num(productWidth, 3)} x ${num(productLength, 3)} in`,
    die: `${num(dieWidth, 3)} x ${num(dieLength, 3)} in`
  };
}

function dieDimensionWarningMarkup() {
  const mismatch = dieDimensionMismatch();
  if (!mismatch) return "";
  return issueList("Advertencia de dimensiones", [
    `Las medidas del troquel (${mismatch.die}) no corresponden con las dimensiones del producto (${mismatch.product}).`
  ]);
}
...
function applyDieDefaults(dieCode) {
  ...
  state.form.troquel.dieShape = dieShapeOptionValue(metricsValue.dieShape || state.form.troquel.dieShape);
...
function bindProcesses() {
  ...
  const update = (event, shouldRender = false) => {
    ...
    if (scope === "troquel" && (field === "dieShape" || field === "associateDieShape") && state.form.troquel.associateDieShape !== false && state.form.troquel.dieCode) {
      const selectedDie = findDie(state.form.troquel.dieCode);
      if (selectedDie && !dieMatchesShape(selectedDie, state.form.troquel.dieShape)) state.form.troquel.dieCode = "";
    }
    if (scope.startsWith("printStages.")) syncPrimaryPrintStage();
  ...
  els.processSections.addEventListener("click", (event) => {
    const shapeTrigger = event.target.closest("[data-calc-shape-trigger]");
    if (shapeTrigger) {
      event.preventDefault();
      event.stopPropagation();
